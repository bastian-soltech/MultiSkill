...
export default function LoginPage() {
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center p-4 sm:p-6 transition-colors duration-500 ${theme.bg} ${theme.font}`}>
      ...
      >
        <h2 className={`text-3xl sm:text-4xl mb-2 ${theme.heading} ${theme.text}`}>{isRpgMode ? 'RETURN TO QUEST' : 'Welcome Back'}</h2>
        <p className={`${theme.muted} mb-10`}>{isRpgMode ? 'Resume your legendary journey.' : 'Continue your journey to mastery.'}</p>
        ...
        <div className="relative mb-8">
          ...
          <div className={`relative flex justify-center text-[10px] uppercase font-black tracking-widest ${theme.muted}`}><span className={`${isRpgMode ? 'bg-slate-900' : 'bg-white'} px-4`}>Or continue with</span></div>
        </div>
        ...
        <p className={`mt-10 text-center text-[10px] uppercase font-black tracking-widest ${theme.muted}`}>
          No account? <Link href="/register" className={`hover:underline ${isRpgMode ? 'text-emerald-400' : 'text-indigo-600'}`}>Register here</Link>
        </p>
      </motion.div>
    </div>
  );
